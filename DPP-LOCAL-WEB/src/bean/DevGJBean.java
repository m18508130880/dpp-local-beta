...
public class DevGJBean extends RmiBean
{
	...
	/**
	 * 获取状态 RealStatus、doDefence、doRightClick RealStatus:当前状态 doDefence:左点击查看接口
	 * doRightClick:右点击事件
	 * 
	 * @param request
	 * @param response
	...
	public void WaterAcc(HttpServletRequest request, HttpServletResponse response, Rmi pRmi, boolean pFromZone) throws ServletException, IOException
	{
		...
		String path = "/www/DPP-LOCAL-BETA/DPP-LOCAL-WEB/files/analogData/";
		File file = new File(path);
		File[] array = file.listFiles();
		ArrayList<WaterAccBean> WaterAcc = new ArrayList<WaterAccBean>();
		
		if(currStatus.getFunc_Type_Id().substring(0,2).equals("YJ"))
		{
			...
			{
				currStatus.setJsp("Analog_OL_Water_Map.jsp?Sid=" + Sid + "&Project_Id="+Project_Id);
			}
			for (int i = 0; i < array.length; i++)
			{
						...
						&& array[i].getName().substring(7, 9).equals("YJ"))
				{
					...
					WaterAccList = waterAccBean.analog_Y(FileName.substring(0,12));
					...
					{
						WaterAccBean waterBean = new WaterAccBean();
						waterBean.setSysId(WaterAccList.substring(7, 12));
		...
		//污井
		else if(currStatus.getFunc_Type_Id().substring(0,2).equals("WJ"))
		{
			if(currStatus.getFunc_Type_Id().equals("WJ"))
			{
				currStatus.setJsp("Analog_Sewage_Map.jsp?Sid=" + Sid + "&Project_Id="+Project_Id);
			}else
			{
				currStatus.setJsp("Analog_OL_Sewage_Map.jsp?Sid=" + Sid + "&Project_Id="+Project_Id);
			}
			for (int i = 0; i < array.length; i++)
			{
				if (array[i].getName().substring(0, 6).equals(Project_Id)
						&& array[i].getName().substring(7, 9).equals("WJ"))
				{
					...
					String FileName = array[i].getName();
					WaterAccBean waterAccBean = new WaterAccBean();
					SewageAccList = waterAccBean.analog_W(FileName.substring(0,12));
					...
					{
						WaterAccBean waterBean = new WaterAccBean();
						waterBean.setSysId(SewageAccList.substring(7, 12));
						waterBean.setStatus("1");
						WaterAcc.add(waterBean);
					}
				}
	...
	public void GJExcel(HttpServletRequest request, HttpServletResponse response, Rmi pRmi, boolean pFromZone, ServletConfig pConfig)
	{
		try
		{
			...
			if (mySmartUpload.getFiles().getCount() > 0 && mySmartUpload.getFiles().getFile(0).getFilePathName().trim().length() > 0)
			{
				...
				{
					String FileSaveRoute = "/www/DPP-LOCAL-BETA/DPP-LOCAL-WEB/files/upfiles/";
					// 上传现有文档
	...
	public void DetailSenceUp(HttpServletRequest request, HttpServletResponse response, Rmi pRmi, boolean pFromZone, ServletConfig pConfig) throws ServletException, IOException
	{
		try
		{
			...
			if (mySmartUpload.getFiles().getCount() > 0 && mySmartUpload.getFiles().getFile(0).getFilePathName().trim().length() > 0)
			{
				if (mySmartUpload.getFiles().getFile(0).getSize() / 1024 <= 3072)// 最大3M
				{
					String FileSaveRoute = "/www/DPP-LOCAL-BETA/DPP-LOCAL-WEB/skin/images/GJ_Img/";
					com.jspsmart.upload.File myFile = mySmartUpload.getFiles().getFile(0);
					String Img_Url = "";
					switch(Cmd){
						case 14:
	...
	public void ConvertLatAndLng(HttpServletRequest request, HttpServletResponse response, Rmi pRmi, boolean pFromZone) throws IOException
	{
		getHtmlData(request);
		currStatus = (CurrStatus) request.getSession().getAttribute("CurrStatus_" + Sid);
		currStatus.getHtmlData(request, pFromZone);
		...
		currStatus.setTotalRecord(msgBean.getCount());
		ArrayList DevGJ_Info = (ArrayList) msgBean.getMsg();
		if(null != DevGJ_Info)
		{
			ProjectInfoBean projectInfoBean = new ProjectInfoBean();
			projectInfoBean.setId(Project_Id);
			projectInfoBean.setCoord(request.getParameter("Coord"));
			msgBean = pRmi.RmiExec(12, projectInfoBean, 0, 25);
			
			int succCnt = 0;
			Iterator deviter = DevGJ_Info.iterator();
			while(deviter.hasNext())
			{
				DevGJBean dBean = (DevGJBean)deviter.next();
				
				JSONObject jsonObject = CoordConv.BD2_GCJ(dBean.getLatitude(),dBean.getLongitude());
				dBean.setProject_Id(Project_Id);
				dBean.setLatitude(jsonObject.getString("Lat"));
				dBean.setLongitude(jsonObject.getString("Lng"));				
				msgBean = pRmi.RmiExec(19, dBean, 0, 25);
				if(MsgBean.STA_SUCCESS == msgBean.getStatus())
					succCnt ++;
			}
			
			currStatus.setResult("成功更新[" + String.valueOf(succCnt) + "]个坐标");
		}else
		{
			currStatus.setResult("更新失败！");
		}
	 ...
	 * 获取相应sql语句
	 * 
	 * @see rmi.RmiBean#getSql(int) 返回的是一个字符串: sql语句
	 */
	public String getSql(int pCmd)
	{
		...
		{
			case 0:// 查询（类型&项目）
				Sql = " select t.id, t.Longitude, t.latitude, t.top_Height, t.base_height, t.Size, t.in_id, t.out_id, t.Material, t.Flag, t.Data_Lev, round((t.curr_data),2) , t.sign , t.project_id, t.project_name, t.equip_id ,t.equip_name ,t.equip_height, t.In_Img, t.Out_Img" +
						" from view_dev_gj t " + 
						"where t.id like '%" + currStatus.getFunc_Sub_Type_Id() + "%' " + " and t.project_id = '" + currStatus.getFunc_Project_Id() + "' " + " order by t.id  ";
				break;
			case 1:// 查询（全部）
				Sql = " select t.id, t.Longitude, t.latitude, t.top_Height, t.base_height, t.Size, t.in_id, t.out_id, t.Material, t.Flag, t.Data_Lev, round((t.curr_data),2) , t.sign , t.project_id, t.project_name, t.equip_id ,t.equip_name ,t.equip_height, t.In_Img, t.Out_Img" + 
						" from view_dev_gj t " + " order by t.id  ";
				break;
			case 2:// 查询（项目）
				Sql = " select t.id, t.Longitude, t.latitude, t.top_Height, t.base_height, t.Size, t.in_id, t.out_id, t.Material, t.Flag, t.Data_Lev, round((t.curr_data),2) , t.sign , t.project_id, t.project_name, t.equip_id ,t.equip_name ,t.equip_height, t.In_Img, t.Out_Img" + 
						" from view_dev_gj t " + 
						" where t.project_id = '" + currStatus.getFunc_Sub_Type_Id() + "'" + " order by t.id";
				break;
			...
			case 6:
				Sql = " select t.id, t.Longitude, t.latitude, t.top_Height, t.base_height, t.Size, t.in_id, t.out_id, t.Material, t.Flag, t.Data_Lev, round((t.curr_data),2) , t.sign , t.project_id, t.project_name, t.equip_id ,t.equip_name ,t.equip_height, t.In_Img, t.Out_Img" + 
						" from view_dev_gj t " + 
						" where t.id = '" + Id + "' and t.project_id = '" + currStatus.getFunc_Project_Id() + "'" + " order by t.id  ";
				break;
			case 4:// 查询多个(项目&id）
				Sql = " select t.id, t.Longitude, t.latitude, t.top_Height, t.base_height, t.Size, t.in_id, t.out_id, t.Material, t.Flag, t.Data_Lev, round((t.curr_data),2) , t.sign , t.project_id, t.project_name, t.equip_id ,t.equip_name ,t.equip_height, t.In_Img, t.Out_Img" + 
						" from view_dev_gj t " + 
						" where instr('" + Id + "', t.id) > 0 and t.project_id = '" + currStatus.getFunc_Project_Id() + "'" + " order by t.id  ";
				break;
			case 5:// 查询（项目&子系统）
				Sql = " select t.id, t.Longitude, t.latitude, t.top_Height, t.base_height, t.Size, t.in_id, t.out_id, t.Material, t.Flag, t.Data_Lev, round((t.curr_data),2) , t.sign , t.project_id, t.project_name, t.equip_id ,t.equip_name ,t.equip_height, t.In_Img, t.Out_Img" + 
						" from view_dev_gj t " + 
						" where t.project_id = '" + Project_Id + "'" + " and substr(t.id, 3, 3) = '" + Subsys_Id + "'" + " order by t.id";
				break;
			case 10:// 添加
				...
				break;
			case 11:// 编辑
				Sql = " update dev_gj t set t.in_id= '" + In_Id + "', t.out_id = '" + Out_Id + "' ,t.top_height= '" + Top_Height + "', t.base_height = '" + Base_Height + "', t.size = '" + Size + "', t.Flag = '" + Flag + "', t.Data_Lev = '" + Data_Lev + "',t.material = '" + Material + "',t.equip_height = '" + Equip_Height + "' " + " where t.id = '" + Id + "' and t.project_id = '" + currStatus.getFunc_Project_Id() + "'";
				break;
			case 12:// 删除
			...
			case 18:// 窨井外图更新
				...
				break;
			case 19:// 经纬度转换
				Sql = "insert into dev_gj(id, latitude, longitude, top_Height, base_height, Size, in_id, out_id, Material, Flag, Data_Lev, project_id, sign) " + 
						"values('" + Id + "','" + Latitude + "','" + Longitude + "','" + Top_Height + "','" + Base_Height + "','" + Size + "','" + In_Id + "','" + Out_Id + "','" + Material + "','" + Flag + "','" + Data_Lev + "','" + Project_Id + "','" + Sign + "')";
				break;
			case 21:// 获取已标注管井
	...
	public boolean getData(ResultSet pRs)
	{
		...
		try
		{
			...
			setIn_Img(pRs.getString(19));
			setOut_Img(pRs.getString(20));
		}
